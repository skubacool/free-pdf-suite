...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Unicode (Thai/CJK) text support: pdf-lib standard fonts cannot encode Unicode, so
  // when text contains Unicode characters we embed Noto Sans Thai or Noto Sans CJK via fontkit.
  // The fonts are fetched once and cached for the session; subsetting keeps the saved PDF small.
  ...
  // Thai needs NO manual glyph remapping with the Noto fonts we embed.
  // This previously shifted marks after tall consonants (ป ผ ฝ พ ฟ ฬ) and
  // below-vowels after ญ/ฐ into the Private Use Area (c + 0xF000) — a trick for
  // legacy Thai fonts (Angsana/Cordia) that ship PUA alternates. Noto Sans Thai
  // has none, so e.g. "พื" became U+FE37 => .notdef (blank box), which is exactly
  // why "พื้นที่" rendered broken. fontkit's layout() (which pdf-lib calls) already
  // applies the font's GSUB/GPOS — it swaps in the descender-less ญ/ฐ variants and
  // positions stacked marks — so pass the text through untouched.
  const adjustThai = (str) => str || '';

  // Script-aware font routing. Previously ANY character above U+00FF that was
  // not Thai went to Noto Sans CJK - but that font has no Latin-Extended-A or
  // Greek glyphs, so Polish/Turkish/Czech/Greek text silently became blank
  // boxes. Route by script instead, and fall back to Noto Sans (Latin-Extended,
  // Greek, Cyrillic, Vietnamese) for everything else.
  ...
  const getUnicodeFont = async (doc, text = '') => {
    ...
    let unsupported = [];
    try {
      if (!parsedFonts[pick.url]) parsedFonts[pick.url] = fontkit.create(new Uint8Array(bytes.slice(0)));
      const fk = parsedFonts[pick.url];
      const seen = new Set();
      for (const ch of String(text || '')) {
        const cp = ch.codePointAt(0);
        if (cp <= 0x20 || seen.has(ch)) continue;
        seen.add(ch);
        if (!fk.hasGlyphForCodePoint(cp)) unsupported.push(ch);
      }
    } catch (_) { unsupported = []; }
    // Subsetting is set PER FONT because pdf-lib's subsetter behaves differently
    // for each (all verified by rendering the output, not by assumption):
    //   Thai  (47KB)   subset:true  - correct; subset:false drops glyphs (gaps mid-word)
    //   CJK   (15.7MB) subset:true  - correct for CJK, and far too big to embed whole
    //   Noto Sans (543KB) subset:false - subset:true renders Latin-Extended/Greek/
    //     Cyrillic blank, so this one is embedded intact
    const embedded = await doc.embedFont(bytes, { subset: pick.subset });
    try { embedded.__unsupported = unsupported; } catch (_) {}
    return embedded;
  };

  // Global scope helper definitions for landing page compatibility
  ...
  const getRotatedOrigin = (Vx, Vy, w, h, page) => {
    ...
    let cx, cy, pdfRotate;
    if (angle === 90) {
      cx = Vy; cy = Vx; pdfRotate = 90;
    } else if (angle === 180) {
      cx = pw - Vx; cy = Vy; pdfRotate = 180;
    } else if (angle === 270) {
      cx = pw - Vy; cy = ph - Vx; pdfRotate = 270;
    } else {
      cx = Vx; cy = ph - Vy; pdfRotate = 0;
    }
    
    const rad = pdfRotate * Math.PI / 180;
    const dx = (w/2) * Math.cos(rad) - (h/2) * Math.sin(rad);
    const dy = (w/2) * Math.sin(rad) + (h/2) * Math.cos(rad);
    
    return { x: cx - dx, y: cy - dy, rotate: degrees(pdfRotate) };
  };
...
setTimeout(() => { try { page.cleanup(); } catch(e){} }, 0);
  ...
  // Removed local wrapText (moved to global helper section)
        ...
        if (pos === 'tile') {
          ...
          for (let yy = -size; yy < vh + stepY; yy += stepY) {
            for (let xx = -tw; xx < vw + tw; xx += stepX) {
              ...
              const center = getRotatedOrigin(VxCenter, VyCenter, 0, 0, page);
              const rad = (angle + 45) * Math.PI / 180;
              const dx = (tw/2) * Math.cos(rad) - (size/2) * Math.sin(rad);
              const dy = (tw/2) * Math.sin(rad) + (size/2) * Math.cos(rad);
              page.drawText(text, { x: center.x - dx, y: center.y - dy, size, font, color, opacity, rotate: degrees(angle + 45) });
            }
          }
  ...
  if ($('#dz-scanned')) {
    ...
    $('#btn-scanned').addEventListener('click', async () => {
      ...
      try {
        ...
        const dpi = +$('select#res-scanned')?.value || 150;
...
setTimeout(() => { try { page.cleanup(); } catch(e){} }, 0);
  ...
  if ($('#panel-compare')) {
    ...
    $('#btn-compare').addEventListener('click', async () => {
      ...
      try {
        ...
        container.innerHTML = '';
        $('div#res-compare').classList.remove('hidden');
        for (let i = 1; i <= maxPages; i++) {
          ...
          const wDiff = cDiff.width;
          const hDiff = cDiff.height;
          const wA = cA.width;
          const hA = cA.height;
          const wB = cB.width;
          const hB = cB.height;
          
          for (let y = 0; y < hDiff; y++) {
            for (let x = 0; x < wDiff; x++) {
              const diffIdx = (y * wDiff + x) * 4;
              
              let rA = 255, gA = 255, bA = 255, aA = 255;
              if (x < wA && y < hA) {
                const idxA = (y * wA + x) * 4;
                rA = dA[idxA] !== undefined ? dA[idxA] : 255;
                gA = dA[idxA + 1] !== undefined ? dA[idxA + 1] : 255;
                bA = dA[idxA + 2] !== undefined ? dA[idxA + 2] : 255;
                aA = dA[idxA + 3] !== undefined ? dA[idxA + 3] : 255;
              }
              
              let rB = 255, gB = 255, bB = 255, aB = 255;
              if (x < wB && y < hB) {
                const idxB = (y * wB + x) * 4;
                rB = dB[idxB] !== undefined ? dB[idxB] : 255;
                gB = dB[idxB + 1] !== undefined ? dB[idxB + 1] : 255;
                bB = dB[idxB + 2] !== undefined ? dB[idxB + 2] : 255;
                aB = dB[idxB + 3] !== undefined ? dB[idxB + 3] : 255;
              }
              
              if (Math.abs(rA - rB) > 15 || Math.abs(gA - gB) > 15 || Math.abs(bA - bB) > 15) {
                dDiff[diffIdx] = 239; dDiff[diffIdx+1] = 68; dDiff[diffIdx+2] = 68; dDiff[diffIdx+3] = 255;
              } else {
                const avg = 0.5 * (rA + rB);
                dDiff[diffIdx] = avg; dDiff[diffIdx+1] = avg; dDiff[diffIdx+2] = avg; dDiff[diffIdx+3] = 60;
              }
            }
          }
  ...
  if ($('#dz-extractfiles')) {
    ...
    $('#btn-extractfiles').addEventListener('click', async () => {
      ...
      try {
              ...
              if (node.has(PDFName.of('Names'))) {
                ...
                for (let idx = 0; idx < namesArray.size(); idx += 2) {
                  ...
                  if (fileSpec.has(PDFName.of('EF'))) {
                    ...
                    const streamKey = efDict.has(PDFName.of('UF')) ? PDFName.of('UF') : (efDict.has(PDFName.of('F')) ? PDFName.of('F') : null);
                    if (streamKey) {
                      const stream = efDict.lookup(streamKey, PDFStream);
        ...
        $('div#res-extractfiles').classList.remove('hidden');
    ...
    $('#btn-inspect').addEventListener('click', async () => {
      ...
      try {
        ...
        for (let i = 1; i <= pdfjsDoc.numPages; i++) {
          ...
          await page.getOperatorList(); // Ensure operator list is loaded and commonObjs is populated
          const fontList = typeof page.commonObjs.keys === 'function' ? page.commonObjs.keys() : (page.commonObjs.keys || []);
          fontList.forEach(f => {
            if (f.startsWith('g_d')) {
        ...
        $('div#res-inspect').classList.remove('hidden');
        setStatus('inspect', '');
      } catch (err) {
  ...
  if ($('#dz-imgocr')) {
    ...
    $('#btn-imgocr').addEventListener('click', async () => {
      ...
      try {
        ...
        $('div#res-imgocr').classList.remove('hidden');
  ...
  if ($('#dz-imgpalette')) {
    ...
    $('#btn-imgpalette').addEventListener('click', async () => {
      ...
      try {
        ...
        $('div#res-imgpalette').classList.remove('hidden');
  ...
  if ($('#dz-letterhead')) {
    ...
    // Removed pdfgrid tool (moved to src/tools/pdfgrid.js)

  // ---- Merge images into one image (side-by-side / stacked / grid) ----
  // Distinct from JPG-to-PDF and the PDF collage: the output here is a single
  // combined IMAGE file, not a PDF.
  if ($('#dz-imgmerge')) {
    let files = [];
    const refresh = () => {
      $('#picked-imgmerge').textContent = files.length
        ? `${files.length} image${files.length > 1 ? 's' : ''} selected — merged in the order added.`
        : '';
      $('#btn-imgmerge').disabled = files.length < 2;
    };
    setupDropzone('imgmerge', (fs) => {
      const imgs = fs.filter(isImageFile);
      if (!imgs.length) { setStatus('imgmerge', '❌ Please choose JPG, PNG or WebP images.', 'error'); return; }
      files.push(...imgs);
      hideResult('imgmerge'); setStatus('imgmerge', '');
      refresh();
    });
    if ($('#clear-imgmerge')) $('#clear-imgmerge').addEventListener('click', () => {
      files = []; hideResult('imgmerge'); setStatus('imgmerge', 'Cleared — add images again.'); refresh();
    });
    const syncMerge = () => $('#cols-wrap-imgmerge').classList.toggle('hidden', $('#dir-imgmerge').value !== 'grid');
    $('#dir-imgmerge').addEventListener('change', syncMerge); syncMerge();
    $('#btn-imgmerge').addEventListener('click', async () => {
      const btn = $('#btn-imgmerge'); btn.disabled = true; hideResult('imgmerge');
      try {
        if (files.length < 2) throw new Error('Add at least two images to merge.');
        const dir = $('#dir-imgmerge').value;
        const gap = Math.max(0, +$('#gap-imgmerge').value || 0);
        const bg = $('#bg-imgmerge').value || '#ffffff';
        const type = $('#fmt-imgmerge').value === 'png' ? 'image/png' : 'image/jpeg';
        const transparent = type === 'image/png' && $('#transparent-imgmerge') && $('#transparent-imgmerge').checked;
        const imgs = [];
        for (let i = 0; i < files.length; i++) {
          setStatus('imgmerge', `Loading image ${i + 1} of ${files.length}…`);
          imgs.push(await loadImageEl(files[i]));
        }
        // Vertical matches every image to the widest, horizontal to the tallest,
        // grid fits each image inside a uniform centred cell.
        let W = 0, H = 0; const place = [];
        if (dir === 'horizontal') {
          H = Math.max(...imgs.map((i) => i.naturalHeight));
          let x = 0;
          imgs.forEach((im, idx) => {
            const w = im.naturalWidth * (H / im.naturalHeight);
            place.push({ im, x, y: 0, w, h: H });
            x += w + (idx < imgs.length - 1 ? gap : 0);
          });
          W = x;
          ...
          imgs.forEach((im, idx) => {
            ...
            place.push({ im, x: 0, y, w: W, h });
            y += h + (idx < imgs.length - 1 ? gap : 0);
          });
          H = y;
        } else {
          const cols = Math.max(1, Math.min(10, +$('#cols-imgmerge').value || 2));
          const cw = Math.max(...imgs.map((i) => i.naturalWidth));
          const ch = Math.max(...imgs.map((i) => i.naturalHeight));
          ...
          imgs.forEach((im, idx) => {
            ...
            place.push({ im, x: c * (cw + gap) + (cw - w) / 2, y: r * (ch + gap) + (ch - h) / 2, w, h });
          });
        }
        // Browsers refuse to allocate absurd canvases — fail with a clear message.
        if (W * H > 80000000) throw new Error('The combined image would be too large. Use fewer images, or resize them smaller first.');
        const c = document.createElement('canvas');
        c.width = Math.max(1, Math.round(W)); c.height = Math.max(1, Math.round(H));
        const ctx = c.getContext('2d');
        if (!transparent) { ctx.fillStyle = bg; ctx.fillRect(0, 0, c.width, c.height); }
        ctx.imageSmoothingEnabled = true; ctx.imageSmoothingQuality = 'high';
        setStatus('imgmerge', 'Merging…');
        place.forEach((p) => ctx.drawImage(p.im, p.x, p.y, p.w, p.h));
        const blob = await canvasBlob(c, type, type === 'image/png' ? undefined : 0.92);
        await deliverImages('imgmerge', [{ blob, name: 'merged' + extFor(type) }], '');
